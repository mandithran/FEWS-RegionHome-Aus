...
"""df_mwl = df.groupby(['rowInd'])['colIndWater'].agg(
        [('colIndWater', 'max')])
print(df_mwl["rowInd"])
print(df_mwl.head())
merged_dfWat = pd.merge(df[["rowInd","colIndWater","geometry"]],
                        df_mwl[["rowInd","colIndWater"]],
                        left_on = "colIndWater",
                        how= "left")"""

#piv = df.pivot_table(columns='colIndWater').reset_index()
#print(piv)


################ Determine extreme erosion scarp line from XBeach output ################