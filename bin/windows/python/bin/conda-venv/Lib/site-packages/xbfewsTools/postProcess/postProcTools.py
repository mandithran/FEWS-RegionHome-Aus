...
def compute_scw(ewlDistSeries=None):
    ...
    bins.insert(0,0)
    scw_series = pd.cut(ewlDistSeries, bins, labels = thresholdsSCW_df["level"]).astype(str)
    #============== Label instances where lines intersect bounds ==============#
    # If they intersect, algorithm above throws a "nan"
    scw_series = scw_series.replace('nan',"High")
    return scw_series


def compute_bsd(bsdDistSeries=None):
    ...
    bins.insert(0,0)
    bsd_series = pd.cut(bsdDistSeries, bins, labels = thresholdsBSD_df["level"]).astype(str)
    #============== Label instances where lines intersect bounds ==============#
    # If they intersect, algorithm above throws a "nan"
    bsd_series = bsd_series.replace('nan',"High")
    return bsd_series