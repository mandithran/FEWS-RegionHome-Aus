...
for d in np.arange(0,ds["pointtime"].sizes["pointtime"],1):
    ...
    gdf.to_file(os.path.join(gaugesDir,"gauges_%ss.shp" % time_str))
    # Export line shapefile
    gdf2 = gpd.GeoSeries(LineString(gdf.geometry.tolist()),crs=epsg)
    gdf2.to_file(os.path.join(gaugesDir,"gauges_%ss_lines.shp" % time_str))
...
for t in np.arange(0,ds["pointtime"].sizes["pointtime"],1):
    ...
    print(len(ds_sub['point_xz'].values))
...
gdf3.to_file(os.path.join(gaugesDir,"ewl_XBeach.shp"))
