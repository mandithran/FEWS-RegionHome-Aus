...
def nearGeom(point, pts=None, gdfIn=None, outVar=None):
    """
    Finds the nearest point and returns the corresponding values.
    point: shapely Point object, the point of interest
    pts: shapely MultiPoint object, the points to be searched with "point"
    gdfIn: same as pts but a GeoDataFrame object. This is provided
           so that geometries can be looked up using the indeces.
    outVar: variable to return. Can return the index or the geometry
    For more info on how these are defined, see the "extractAusWavePts"
    function defined in this file
    """
    # find the nearest point and return the corresponding index value
    # nearest is a pandas boolean series. "True" if the point is the nearest
...
def parse_BOMWaveFile(dt=None,waveCode=None):
    """
    Function for parsing the file name of the BoM wave forecast,
    using the forecast rounded time and the wave code for a 
    given hotspot location. 

    INPUTS: 
    dt: datetime of the rounded time, in python datetime format.
        Used to download the correct forecast at the direct time.
    waveCode: Wave code for a given hotspot, set in [Region Home]\
        [Config]\MapLayerFiles\hotspotLocations.csv

    OUTPUTS:
    fname: The full file name of the BoM wave forecast
    bomDate: Date of the file being downloaded, formatted as a
        string
    bomTime: Time of the file being downloaded, formatted as a
        string
    """
