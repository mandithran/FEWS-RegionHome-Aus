...
df = pd.read_csv(os.path.join(oDir, "WL-NTR-obsAndpred_Narrabeen_2020_NoNTS.csv"))
...
fig.suptitle("Observed vs. predicted non-tidal residuals (storm surge) - 2020",
             fontsize=10,y=1)
figName = 'ObsVPred_%s_%s-NoNTS.png' % (siteName, varString)
...
fig.tight_layout()
fig.subplots_adjust(top=0.94,hspace=.38)
fig.suptitle("Observed vs. predicted total water levels (just tides, no surge) - 2020",
             fontsize=10,y=1)
figName = 'ObsVPred_%s_%s-NoNTS.png' % (siteName, varString)
fig.delaxes(axes[2,1])
#plt.show()
