...
plotPeriods = [datetime(2020,5,2,0),
...
def generateTimeseriesPlot(fig=None, ax=None, df=None):
    ...
    ax.set_title("%s" % (key))
    # Legend
    ax.legend(['observed water level','predicted tide + surge'],loc='upper left')
...
# Fig params
fig, axes = plt.subplots(4,1,figsize=(4,8),sharex=False)
fig.subplots_adjust(top=0.94,hspace=.7)
axs = np.array(axes).reshape(-1)

# Plot time series
variable = "surge + tide (m)"
counter = 0
