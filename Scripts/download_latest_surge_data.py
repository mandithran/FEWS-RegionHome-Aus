...
#                                   Paths                                    #
#============================================================================#
localDir = "C:\\Users\z3531278\\Documents\\01_FEWS-RegionHome-Aus\\ExternalForecasts\\BOM\\WaterLevel"
...
# I want this script to run every day and download all the storm surge forecasts available for the previous day
# This is now for archival purposes so all the intricate checking doesn't really matter
# If the file exists I will just overwrite it
# But if there's a shutdown I need the script to download all the missed files
# In which case I need to retain certain files locally and do a check to see which ones I need to download
...
# Case 2 - Files exist with gaps; parse times and set download_start_date to right before earliest gap 
    ...
    # Check for files greater than 2 weeks (### hrs). Delete those. Assumes no gaps around this time.
    # 2 week period selected due to local storage
    df_del = df[df["Time"] < (todaysdate - timedelta(days=14))]
    delnp = df_del["FileName"].to_numpy()
    for f in delnp:
        os.remove(os.path.join(localDir,f))
    df = df[df["Time"] > (todaysdate - timedelta(days=14))]
...
for i,text in enumerate(soup_split):
    ...
    if download_flag:
        ...
        file = wget.download(url, out=localDir) 
